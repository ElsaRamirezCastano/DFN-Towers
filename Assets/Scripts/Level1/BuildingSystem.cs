...
public class BuildingSystem : MonoBehaviour
{
    ...
    {
        TileBase currentTile = MainTilemap.GetTile(position);
        if (currentTile == takenTile)
        {
            return false;
        }
    ...
    {
        return PathTilemap.GetTile(position) == pathTile;
    }
    ...
    public void ClearAllPaths()
    {
        ...
        for (int x = bounds.xMin; x < bounds.xMax; x++)
        {
            for (int y = bounds.yMin; y < bounds.yMax; y++)
            {
                ...
                if (PathTilemap.GetTile(position) == pathTile)
                {
                    PathTilemap.SetTile(position, null);
                }
            }
    ...
    private void HandlePathBuilding()
    {
        ...
        {
            if (PathTilemap.GetTile(lastPreviewCell) != pathTile)
            {
                PathTilemap.SetTile(lastPreviewCell, null);
            }
        }
        ...
        {
            PathTilemap.SetTile(currentCell, pathPreviewTile);
            lastPreviewCell = currentCell;
        }
        else if (IsPathAt(currentCell))
        {
            lastPreviewCell = currentCell;
        }
        else
